...
public class GameNativeAndroid: GameNativeBase
{
	...
	public override void showMessageOkCancel(string title = "", string message = "", string okButton = "", string cancelButton = "")
	{
		if(title.IsEmpty())
		{
			title = Flow.messageOkCancelDialog.transform.FindChild("BackgroundBorder").FindChild("BackgroundTitle").FindChild("TitlePanel").FindChild("Title").GetComponent<SpriteText>().Text;
		}
		
		if(message.IsEmpty())
		{
			message = Flow.messageOkCancelDialog.transform.FindChild("BackgroundBorder").FindChild("BackgroundBody").FindChild("MessagePanel").FindChild("Message").GetComponent<SpriteText>().Text;
		}
		...
		if(okButton.IsEmpty())
		{
			try
			{
				okButton = Flow.messageOkCancelDialog.transform.FindChild("BackgroundBorder").FindChild("BackgroundBody").FindChild("ConfirmButtonPanel").FindChild("ConfirmButton").FindChild("control_text").GetComponent<SpriteText>().Text;
			}
			catch
			{
		...
		if(cancelButton.IsEmpty())
		{
			try
			{
				cancelButton = Flow.messageOkCancelDialog.transform.FindChild("BackgroundBorder").FindChild("BackgroundBody").FindChild("CancelButtonPanel").FindChild("CancelButton").FindChild("control_text").GetComponent<SpriteText>().Text;
			}
			catch
			{

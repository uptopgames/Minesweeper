...
public class Multiplayer : MonoBehaviour 
{
	...
	void OnReceiveGames(string error, IJSonObject data)
	{
		...
		else 
		{
			...
			for(int i = 0 ; i < data["games"].Count ; i++)
			{
				...
				string[] bangTimes = new string[Flow.ROUNDS_PER_TURN];
				string[] shotTimes = new string[Flow.ROUNDS_PER_TURN];
				string[] sandAttacks = new string[Flow.ROUNDS_PER_TURN];
				string[] pastMyBangTimes = new string[Flow.ROUNDS_PER_TURN];
				string[] pastMyShotTimes = new string[Flow.ROUNDS_PER_TURN];
				string[] pastTheirBangTimes = new string[Flow.ROUNDS_PER_TURN];
				string[] pastTheirShotTimes = new string[Flow.ROUNDS_PER_TURN];
				
				//string[] pastMyShotTimes = new string[Flow.ROUNDS_PER_TURN];
				//string[] pastMyBangTimes = new string[Flow.ROUNDS_PER_TURN];
				string[] pastMySandAttacks = new string[Flow.ROUNDS_PER_TURN];
				//string[] pastTheirShotTimes = new string[Flow.ROUNDS_PER_TURN];
				//string[] pastTheirBangTimes = new string[Flow.ROUNDS_PER_TURN];
				string[] pastTheirSandAttacks = new string[Flow.ROUNDS_PER_TURN];
				string[] pastMyRoundsWin = new string[Flow.ROUNDS_PER_TURN];
				string[] pastTheirRoundsWin = new string[Flow.ROUNDS_PER_TURN];
				...
				if(data["games"][i]["turnStatus"].StringValue != "newturn" && data["games"][i]["whoseMove"].StringValue != "their")
				{
				...
				if(!data["games"][i]["lastTurn"].IsNull) 
				{
					...
					pastMyRoundsWin = data["games"][i]["myPastRoundsWin"].StringValue.Split(',');
					pastTheirRoundsWin = data["games"][i]["theirPastRoundsWin"].StringValue.Split(',');
			...
			if(data["games"].Count > 0)
			{
				...
				if(oldTheirTurnNumber > 0 && Flow.theirTurnGames == 0)
				{
					...
					if(oldYourTurnNumber > 0) scroll.RemoveItem(Flow.yourTurnGames+1,true);
	...
	/*GameObject CreateGameContainer (IJSonObject game)
	{
		...
		if(game["turnStatus"].StringValue != "waitingChoice" && game["whoseMove"].StringValue != "their")
		{
			...
			for(int j = 0 ; j < Flow.ROUNDS_PER_TURN ; j++)
			{
				tempRoundList.Add (new Round (-1, game["turn"].Int32Value, game["friendID"].Int32Value, times[j].ToFloat(), scores[j].ToInt32()));
			}
		}
		if(!game["lastTurn"].IsNull) 
		{
			...
			{
				tempPastMyRoundList.Add(new Round(-1,-1,-1,pastMyTimes[k].ToFloat(),int.Parse(pastMyScores[k]), -1, -1));
				tempPastTheirRoundList.Add(new Round(-1,-1,-1,pastTheirTimes[k].ToFloat(),int.Parse(pastTheirScores[k]), -1, -1));
			}
		}
		...
		tempGameContainer.GetComponent<Game>().SetGame(game["gameID"].Int32Value,tempGameContainer.GetComponent<Friend>(),game["world"].Int32Value,
			game["worldName"].StringValue, game["level"].Int32Value, new List<Round>(),tempRoundList,game["turn"].Int32Value,lastTurnID,game["turnsWon"].Int32Value,

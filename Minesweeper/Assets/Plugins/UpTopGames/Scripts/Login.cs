...
public class Login : MonoBehaviour 
{
	...
	public SpriteText facebookCoins;
	...
	void Start ()
	{
		...
		GetComponent<UIInteractivePanel>().transitions.list[0].AddTransitionStartDelegate(EnteredLogin);
	}
	
	void EnteredLogin(EZTransition transition)
	{
		Flow.header.transform.FindChild("OptionsPanel").gameObject.SetActive(true);
		facebookCoins.Text = "+"+Flow.config.GetComponent<ConfigManager>().shopFeatures.coinsFacebook;
	}
	...
	private void connectionResult(string error, WWW data)
	{
		...
		else if(json.Contains("ask") && json["ask"].ToString() == "wrong_password")
		{
			passwordDialog.SetActive(true);
			UIManager.instance.FocusObject = passwordField;
			passwordDialog.transform.FindChild("BackgroundBorder").FindChild("BackgroundTitle").FindChild("TitlePanel").FindChild("Title").GetComponent<SpriteText>().Text = "Wrong password";// If you don't remember it, check your email or email us at admin@uptopgames.com";
	...
	void CancelPasswordDialog()
	{
		...
		passwordField.Text = "";
		password = "";
		passwordDialog.transform.FindChild("BackgroundBorder").FindChild("BackgroundTitle").FindChild("TitlePanel").FindChild("Title").GetComponent<SpriteText>().Text = "Type your password";
	}
